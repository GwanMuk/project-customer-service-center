			...
			<ul class="pageMaker">
				...
				<c:if test="${pageMaker.prev}">
					<li class="pageMaker_btn prev"><a
						href="/admin/authorManage?pageNum=${pageMaker.pageStart - 1}&amount=${pageMaker.cri.amount}">이전</a></li>
				</c:if>
				<!-- 페이지 번호 -->
				<c:forEach begin="${pageMaker.pageStart}" end="${pageMaker.pageEnd}"
					...
					<li
						...
						<a
						href="/admin/authorManage?pageNum=${num}&amount=${pageMaker.cri.amount}">${num}</a>
					</li>
				</c:forEach>
				<!-- 다음 버튼 -->
				<c:if test="${pageMaker.next}">
					<li class="pageMaker_btn next"><a
						href="/admin/authorManage?pageNum=${pageMaker.pageEnd + 1}&amount=${pageMaker.cri.amount}">다음</a></li>
				</c:if>
			</ul>
		</div>
